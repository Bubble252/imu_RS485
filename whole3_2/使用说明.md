...
```bash
# 建立 SSH 隧道并登录服务器（⭐ 新增音频端口 5559/5561）
...
```bash
# 验证 SSH 隧道是否建立（⭐ 现在应该有 6 个端口）
ss -tuln | grep -E "5555|5556|5557|5558|5559|5561"
# 应该看到 6 个端口在 127.0.0.1 上监听
...
python triple_imu_rs485_publisher_dual_cam_UI_voice.py \
    ...
    --ui-command-port 5562  # ⭐ 新增：UI命令接收端口（可选，默认5562）
```
...
python C_real_video_audio.py  # ⭐ 使用音频版本

# 应该能看到（对着麦克风说话）：
# [命令线程] 已接收 10 条命令
# [音频发送线程] 已发送 50 帧, FPS: 16.5, 队列: 2/20  ⭐ 独立音频
# [视频发送线程] 已发送 20 帧, FPS: 7.9, 音频队列: 2  ⭐ 队列不满
```

## 5️⃣ 终端4：运行PyQt5 UI（可选，图形化控制）

⭐ **新增功能：图形化界面控制夹爪 + 音频波形显示**

```bash
cd ~/桌面/WIT_RS485/pyqt5_viewer
python imu_dual_cam_viewer.py --host localhost --port 5560

# UI功能：
# 1. 实时显示IMU数据、3D轨迹、视频流
# 2. 夹爪控制：点击"夹爪控制"面板，按住 '1' 打开，'2' 闭合
# 3. 音频波形：实时显示音频波形和音量
# 4. 与终端2的键盘控制同步（双向控制）
```
