# B_reverse_whole_voice.py - 反向连接版本，支持LeRobot数据集保存和音频流转发
# 功能：
# 1. 接收来自A的控制命令，转发给C
# 2. 接收来自C的JSON数据（包含视频、音频和机器人数据）
# 3. 将数据转换为lerobot格式并保存到本地
# 4. 同时将视频和音频转发给A
# 5. 音频格式：Opus编码（16kHz, 单声道, 24kbps）
import json
import threading
import time
import pickle
...
# B 监听的端口 (让 C 主动连接 - 音频数据流，独立端口)
SERVER_B_PORT_FOR_C_AUDIO = 5559

# B 发布端口 (向 A 发送音频，独立端口)
SERVER_B_PORT_TO_A_AUDIO = 5561
...
class LeRobotDataHandler:
    ...
    def __init__(self, repo_id: str, instruction: str = "", fps: int = 30, 
        ...
        self.fps = fps
        self.executor = ThreadPoolExecutor(max_workers=1) # 用于后台保存
    ...
    def _save_episode_async(self):
        """
        在后台线程中安全地保存episode
        """
        if self.dataset is None or self.frame_count == 0:
            return
        try:
            print(f"💾 开始在后台保存episode (总帧数: {self.frame_count})...")
            self.dataset.save_episode()
            print(f"✅ Episode已成功保存到 {self.dataset.root}")
        except Exception as e:
            print(f"❌ 后台保存episode时出错: {e}")
            import traceback
            traceback.print_exc()
    ...
    def add_frame(self, data_dict: dict):
        ...
        Args:
            data_dict: 包含以下字段的字典:
                - image: 图像数据 (numpy array 或 bytes)
        ...
        try:
            # 处理图像数据（支持多种字段名）
            image_left_wrist = data_dict.get("image.left_wrist") 
            image_top = data_dict.get("image.top")

            if image_left_wrist is None or image_top is None:
                return
            
            # 如果图像是bytes（JPEG编码），需要解码
            if isinstance(image_left_wrist, bytes):
                nparr = np.frombuffer(image_left_wrist, np.uint8)
                image_left_wrist = cv2.imdecode(nparr, cv2.IMREAD_COLOR)

                # BGR转RGB（OpenCV使用BGR，但LeRobot通常使用RGB）
                if len(image_left_wrist.shape) == 3 and image_left_wrist.shape[2] == 3:
                    image_left_wrist = cv2.cvtColor(image_left_wrist, cv2.COLOR_BGR2RGB)
            if isinstance(image_top, bytes):
                nparr = np.frombuffer(image_top, np.uint8)
                image_top = cv2.imdecode(nparr, cv2.IMREAD_COLOR)

                # BGR转RGB
                if len(image_top.shape) == 3 and image_top.shape[2] == 3:
                    image_top = cv2.cvtColor(image_top, cv2.COLOR_BGR2RGB)
            
            # 确保图像是numpy array
            if not isinstance(image_left_wrist, np.ndarray):
                print("⚠️ 警告: 图像格式不正确")
                return
            if not isinstance(image_top, np.ndarray):
                print("⚠️ 警告: 图像格式不正确")
                return
            ...
            task = data_dict.get("instruction")
            ...
            # 准备帧数据
            frame_data = {
                "observation.images.left_wrist": image_left_wrist,
            ...
            if episode_end:
                print(f"📦 Episode结束，提交保存任务 (当前帧数: {self.frame_count})...")
                self.executor.submit(self._save_episode_async)
                self.frame_count = 0
    ...
    def shutdown(self):
        """
        安全关闭，保存任何未完成的episode并关闭线程池
        """
        if self.dataset and self.frame_count > 0:
            print("\n程序关闭中，保存最后一个未完成的episode...")
            # 直接调用，因为程序马上要退出
...
def extract_video_for_forwarding(data_dict: dict, camera_key: str) -> bytes:
    """
    从数据字典中提取视频数据，用于转发给A
    ...
    """
    # 支持多种字段名
    image = data_dict.get(f"image.{camera_key}")
...
def thread_audio_handler():
    """
    线程2：音频流转发 (C -> B -> A)
    独立的音频数据流，不与视频混合
    功能：
    1. 从 C 接收 Opus 编码的音频数据 (端口 5559)
    2. 直接转发给 A (端口 5561)
    支持 C 和 A 断开重连，自动恢复
    """
    context = zmq.Context()
    ...
    socket_from_c.bind(f"tcp://*:{SERVER_B_PORT_FOR_C_AUDIO}")
    socket_from_c.setsockopt(zmq.RCVHWM, 100)  # 高水位标记
    socket_from_c.setsockopt(zmq.LINGER, 0)
    
    # 发送给 A 的音频（PUB模式）
    socket_to_a = context.socket(zmq.PUB)
    socket_to_a.bind(f"tcp://*:{SERVER_B_PORT_TO_A_AUDIO}")
    socket_to_a.setsockopt(zmq.SNDHWM, 100)
    socket_to_a.setsockopt(zmq.LINGER, 0)
    
    print(f"[线程2-音频] 启动音频转发服务")
    print(f"[线程2-音频] 监听 C 音频: *:{SERVER_B_PORT_FOR_C_AUDIO}")
    print(f"[线程2-音频] 转发给 A: *:{SERVER_B_PORT_TO_A_AUDIO}")
    ...
    try:
        while True:
            try:
                # 接收来自 C 的音频数据
                audio_data = socket_from_c.recv()
                received_count += 1
                
                # 直接转发给 A
                socket_to_a.send(audio_data)
                forwarded_count += 1
                ...
                if received_count % 100 == 0:
                    ...
                    print(f"[线程2-音频] 已转发 {forwarded_count} 帧, FPS: {fps:.1f}, "
...
def thread_data_handler(lerobot_handler: LeRobotDataHandler):
    """
    线程3：处理视频数据流 (C -> B -> A)
    反向模式：C 主动连接 B 并推送视频数据
    功能：
    1. 接收来自C的视频数据（不再包含音频，音频走独立端口）
    2. 转换为lerobot格式并保存
    3. 提取视频并转发给A
    支持 C 断开重连，自动恢复
    """
    ...
    while True:
        try:
            ...
            try:
                ...
                has_image = "image.left_wrist" in data_dict or "image.top" in data_dict
                ...
                # 提取视频并转发给A
                video_left_wrist = extract_video_for_forwarding(data_dict, camera_key="left_wrist")
                video_top = extract_video_for_forwarding(data_dict, camera_key="top")

                if video_left_wrist and video_top:
                    # 准备转发给A的数据格式（保持与A的兼容性）
                    video_frame = {
                        "image.left_wrist": video_left_wrist,
                        "image.top": video_top,
                        "encoding": "jpeg",
                    ...
                    # 统计信息
                    video_size = len(video_left_wrist) + len(video_top)
                    print(f"[线程3-视频 B→A] 视频已转发: {video_size} bytes")
                else:
...
def run_server_b(repo_id: str = None, instruction: str = None, fps: int = None,
    ...
    # 启动线程2：音频转发（独立音频流）
    audio_thread = threading.Thread(target=thread_audio_handler, daemon=True)
    audio_thread.start()
    
    # 启动线程3：视频数据处理（包含视频转发和数据集保存）
    data_thread = threading.Thread(target=thread_data_handler, args=(lerobot_handler,), daemon=True)
    data_thread.start()
    
    print("\n三个线程已启动:")
    print("  线程1: 命令转发 (A→C)")
    print("  线程2: 音频转发 (C:5559 → A:5561)")
    print("  线程3: 视频转发 + 数据保存 (C:5558 → A:5557)")
    print("等待 C 连接...")
